...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Desglose de consumo de VRAM por octante según el número máximo de subdivisiones}
...
Se aprecia que, a partir de $10^6$ o $10^7$ subdivisiones, el incremento de VRAM es nulo.
...
Para determinar el valor óptimo, se realizó un análisis de convergencia (Figura~\ref{fig:theta}) empleando una submuestra aleatoria de 10.000 estrellas.
...
Este conjunto de prueba es estadísticamente representativo de la población total, ya que el muestreo cubre uniformemente tanto las regiones de altísima densidad del bulbo como la periferia dispersa.
...
Los resultados dictan la elección de \textbf{$\theta = 0,2$} basándose en tres criterios fundamentales:
...
Utilizando la muestra reducida, se observó una variación relativa de $L_z$ de tan solo \textbf{$1,96 \times 10^{-8}$} tras 5 pasos de simulación.
...
El objetivo es cuantificar la aceleración obtenida mediante infraestructuras de computación de alto rendimiento (HPC) y validar que la paralelización masiva en GPU preserva la integridad científica frente a la ejecución de referencia en CPU.
...
\begin{itemize}
    \item \textbf{Referencia (Fuerza Bruta $O(N^2)$):} La evaluación secuencial directa de la Ley de Gravitación Universal arroja un tiempo medio de \textbf{6.987946 segundos por estrella}. Completar un solo paso para la muestra total requeriría aproximadamente \textbf{235 años}, lo que ilustra la inviabilidad absoluta de este enfoque.
    \item \textbf{Barnes-Hut Secuencial ($O(N \log N)$):} La aplicación del algoritmo de árbol optimiza el tiempo a \textbf{0.0148 segundos por estrella}. En un solo hilo de CPU, el tiempo estimado se reduce a \textbf{183 días}, una mejora crítica pero insuficiente para simulaciones evolutivas multitemporales.
\end{itemize}
La versión paralela con OpenMP ejecutada en un nodo del Finisterrae III utilizando 64 cores reduce el tiempo de ejecución a unas 60 horas, lo que supone una aceleración de 73.49x.
Aunque la verdadera capacidad del sistema se manifiesta al aplicar paralelismo masivo mediante CUDA. Se ha evaluado el tiempo de ejecución de un paso completo de simulación en el nodo \texttt{a100-m8}(nodo del Finisterrae III que dispone de 8 GPUs A100), comparando la ejecución multihilo en CPU (64 núcleos con OpenMP) frente a una configuración de hasta 8 GPUs NVIDIA A100.
Los resultados se detallan en la Tabla~\ref{tab:rendimiento_gpu}.
...
El análisis revela un \textit{speedup} masivo de \textbf{126.7x} al utilizar la capacidad total del nodo.
Siendo la aceleración conseguida con respecto al sistema secuencial de 9.314x, consiguiendo una reducción del tiempo de pared (\textit{wall-clock time}) de casi \textbf{183 días a tan solo 28 minutos}, lo que transforma un problema prohibitivo en un flujo de trabajo manejable.

En cuanto a la escalabilidad del sistema, se aprecia un comportamiento lineal altamente eficiente hasta el uso de 4 GPUs.
No obstante, en el intervalo comprendido entre las 4 y 7 unidades de procesamiento, el rendimiento muestra una meseta técnica atribuible a la propia arquitectura del algoritmo de descomposición espacial.
Dado que el nivel superior del árbol se divide en exactamente ocho octantes, las configuraciones que no son submúltiplos exactos de este número incurren en un desequilibrio de carga (\textit{load imbalance}).
Específicamente, con 4 a 7 GPUs, el sistema se ve obligado a realizar al menos dos iteraciones de cálculo en algunas unidades para cubrir la totalidad de los octantes, provocando que ciertos núcleos permanezcan ociosos mientras otros completan su segundo ciclo de procesamiento.
Este cuello de botella desaparece al alcanzar las 8 GPUs, donde la correspondencia uno a uno entre octantes y unidades de procesamiento permite una ocupación del hardware del 100\%.
Este salto final en la eficiencia maximiza el \textit{throughput} y valida la robustez de la implementación para el procesado masivo de más de mil millones de estrellas.
...
Para validar la implementación, se ha comparado un paso de integración en CPU vs GPU.
Los resultados se analizan a continuación:

\subsubsection{Análisis del error y bits efectivos}
El error medio absoluto (MAE) en posición tras el primer paso es de $6.4460 \times 10^{-05}$ kpc.
...
En términos decimales, esto equivale a una precisión de aproximadamente 5 dígitos significativos de concordancia absoluta.
...
En conclusión, el sistema desarrollado no solo ofrece una aceleración de órdenes de magnitud respecto al estado del arte secuencial, sino que mantiene una fidelidad numérica que garantiza la producción de resultados científicos robustos en el supercomputador Finisterrae III.