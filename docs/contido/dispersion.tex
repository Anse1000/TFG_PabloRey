...
Su objetivo principal es realizar un censo además de un mapa tridimensional cosmológico lo más preciso y completo de las estrellas de la Vía Láctea.
...
Toda esta información proviene del satélite cuyo nombre proviene de la misión que realiza, Gaia (fig.~\ref{fig:sonda}).

Lanzado el 19 de diciembre de 2013 desde la Guyana Francesa, el satélite Gaia se posicionó en una órbita alrededor del Sol a 1,5 millones de Km de la Tierra en un espacio conocido como \textbf{punto de Lagrange L2}.
...
El \textbf{Data Release 3 (DR3)} de la misión Gaia constituye el conjunto de datos más completo y preciso publicado hasta la fecha por la Agencia Espacial Europea~\cite{gaia_dr3_documentation}.
...
El catálogo contiene información astrométrica, fotométrica y espectroscópica de más de \textbf{1.800 millones de objetos}, de los cuales aproximadamente 1.500 millones poseen soluciones astrométricas completas, es decir, posición, paralaje y movimiento propio.
Esta información se almacena en distintas tablas temáticas, a continuación se detallan las más relevantes y necesarias para este proyecto.
Cada fila de estas tablas representa un único objeto observado asociado a un identificador numérico \texttt{source\_id} que permite identificarlo y cruzarlo con distintas tablas y catálogos.
\subsubsection{Tabla \texttt{gaia\_source}}
La tabla \texttt{gaia\_source} contiene los parámetros fundamentales de cada objeto observado ~\cite{gaia_dr3_dm_gaia_source}.
Esta tabla contiene más de cien columnas y constituye la base de las demás tablas, pero para la realización de este proyecto solo son necesarias 9.
\begin{itemize}
    \item \texttt{source\_id}: Identificador común mencionado anteriormente.
    Está construido mediante el sistema de indexación jerárquico HEALPix, lo que permite conocer la localización aproximada de la estrella en el cielo.
    ...
    \item \texttt{ra}: Ascensión Recta ($\alpha$), expresada en grados y referida al sistema de coordenadas ecuatoriales (epoch J2016.0).
    ...
    \item \texttt{dec}: Declinación ($\delta$), también en grados, medida en el sistema ecuatorial.
    ...
    Este valor incluye el factor $\cos(\delta)$, que corrige la convergencia de meridianos hacia los polos.
    ...
    Este parámetro se obtiene a partir del desplazamiento Doppler de las líneas espectrales medidas por el \textit{Radial Velocity Spectrometer} (RVS) a bordo de Gaia.
...
Las columnas extraídas de la tabla \texttt{astrophysical\_parameters} pertenecen al módulo \texttt{FLAME} (\textit{Final Luminosity, Age and Mass Estimator})~\cite{gaia_dr3_dm_astrophysical_parameters}, encargado de estimar parámetros físicos de las estrellas a partir de la información fotométrica y de modelos de evolución estelar.
...
\begin{itemize}
    ...
    En combinación con la temperatura efectiva y la luminosidad, permite situar la estrella dentro del diagrama de Hertzsprung–Russell y estimar su estado evolutivo (secuencia principal, subgigante o gigante).
\end{itemize}
\subsubsection{Tabla adicional}
Además de las columnas anteriores, se incluye también el parámetro \texttt{rgeo}, obtenido a través del servicio \textit{TAP VizieR}, que proporciona datos complementarios derivados de catálogos externos al archivo principal de Gaia~\cite{tap_vizier}.

\begin{itemize}
    \item \texttt{rgeo}: Distancia geométrica estimada en parsecs (pc) basada en el método bayesiano desarrollado por Bailer-Jones.
    ...
    Este parámetro resulta fundamental para convertir magnitudes aparentes en magnitudes absolutas y para estimar luminosidades físicas de las estrellas con mayor precisión.
\end{itemize}

\subsection{Selección y filtrado de los datos}\label{subsec:filtrado}
...
\texttt{bp\_rp}, \texttt{phot\_g\_mean\_mag} o \texttt{rgeo} (distancia geométrica).
...
Antes de realizar el análisis principal de los datos, es necesario obtener una serie de magnitudes físicas y espaciales derivadas a partir de los parámetros disponibles en el catálogo \texttt{Gaia DR3}.
Estos cálculos permiten caracterizar completamente la posición, el movimiento y las propiedades fundamentales de cada estrella en un sistema de referencia coherente.

\subsection{Transformación de coordenadas}\label{subsec:transformacion-de-coordenadas}

Las coordenadas ecuatoriales $(\alpha, \delta)$, propias del sistema de referencia celeste, se transforman a coordenadas galácticas $(X, Y, Z)$ mediante una rotación definida por la orientación del plano galáctico adoptado por la IAU (epoch J2016.0)~\cite{calabretta2006_galactic}.
El sistema resultante tiene su origen en el centro galáctico, el eje $Z$ apunta hacia el norte galáctico, el eje $X$ en dirección al centro, y el eje $Y$ completa la base dextrógira.
Si $d$ es la distancia heliocéntrica de la estrella (en parsecs), las coordenadas cartesianas galácticas (en Kiloparsecs) se obtienen como:
...
\[
                       ...
                       \end{bmatrix}
\] es la matriz de rotación ecuatorial–galáctica adoptada por la IAU, cuyas constantes fueron adoptadas por Murray (1989) y son las utilizadas actualmente por librerías astrométricas como Astropy~\cite{murray1989_coordinates}.
Dado que el sistema de referencia de Gaia DR3 está alineado con el ICRS (epoch J2016.0), esta matriz se mantiene válida para las transformaciones empleadas en este trabajo.
\subsection{Estimación de la Velocidad Radial}\label{subsec:estimacion-de-la-velocidad-radial}

En caso de no estar disponible en el catálogo la velocidad radial para una determinada estrella, se realiza una estimación basada en su posición y movimientos propios, aplicando la formulación clásica de Johnson \& Soderblom (1987)~\cite{johnson1987}.
...
Para su estimación se parte de los movimientos propios en ascensión recta (\( \mu_{\alpha} \)) y declinación (\( \mu_{\delta} \)), junto con la distancia \( d \) a la estrella.
A partir de estos parámetros, se calcula el vector de velocidad tangencial en el espacio tridimensional ecuatorial.

Para determinar la velocidad espacial de la estrella, se construye una matriz de transformación \( T \) que relaciona los movimientos propios con las componentes cartesianas del movimiento:
...
\[
    \vec{V} = \kappa \, d \, (T_1 \, \mu_{\alpha} + T_2 \, \mu_{\delta})
\]
...
\]
En este trabajo se adoptan los valores propuestos por Schönrich et al\. (2010) para el movimiento solar,
y una velocidad de rotación galáctica estándar de \( V_{\text{GAL}} = 220~\text{km s}^{-1} \)~\cite{schonrich2010,blandhawthorn2016}:

\[
    U_{\text{SOL}} = 11.1~\text{km s}^{-1}, \quad
    V_{\text{SOL}} = 12.24~\text{km s}^{-1}, \quad
    W_{\text{SOL}} = 7.25~\text{km s}^{-1}.
\]
...
\(\mu_{\alpha}\cos\delta\) y \(\mu_{\delta}\) (expresados en mas\,año\(^{-1}\)),
junto con una distancia \(d\) (en parsec) y una velocidad radial observada \(V_r\) (en km\,s\(^{-1}\)),
es posible determinar las componentes de la velocidad espacial en el sistema galáctico \((V_X, V_Y, V_Z)\).

El cálculo se basa en la conversión de las magnitudes observadas (coordenadas ecuatoriales) al sistema cartesiano galáctico utilizando de nuevo la formulación de Johnson \& Soderblom (1987)~\cite{johnson1987}.
...
se calculan las componentes de la velocidad en el sistema ecuatorial \((V_X', V_Y', V_Z')\):

\[
    \begin{aligned}
        V_X' &= V_r X_E - V_{t,\alpha}\sin\alpha - V_{t,\delta}\sin\delta\cos\alpha, \\
        V_Y' &= V_r Y_E + V_{t,\alpha}\cos\alpha - V_{t,\delta}\sin\delta\sin\alpha, \\
        V_Z' &= V_r Z_E + V_{t,\delta}\cos\delta,
    \end{aligned}
\]

donde \((X_E, Y_E, Z_E)\) son los cosenos directores de la posición estelar en coordenadas ecuatoriales,
y \(\alpha, \delta\) representan la ascensión recta y la declinación.
...
\section{Cálculos dispersión estelar}\label{sec:dispersion}
...
\subsection{Esquema de integración temporal (Leapfrog, Kick--Drift--Kick).}\label{subsec:esquema-de-integracion-temporal-(leapfrog-kick--drift--kick).}
...
\paragraph{Elección del paso temporal máximo.}
...
Sea \(v_{\max}\) la mayor velocidad galactocéntrica entre todas las estrellas y \(l_{\min}\) la escala espacial mínima resoluble (por ejemplo, el tamaño de celda más pequeño del octree o el parámetro de suavizado \(\varepsilon\)).
...
La contribución debida a las interacciones mutuas entre estrellas se obtiene a partir de la ley de gravitación de Newton\cite{binney2008}.
Para dos partículas puntuales de masas \(m_i\) y \(m_j\) ubicadas en \(\mathbf{r}_i\) y \(\mathbf{r}_j\),
\[
...
\subsubsection{Aproximación multipolar (criterio de Barnes--Hut).}
Para reducir el coste computacional de la suma sobre \(j\), se puede aproximar la contribución de grupos de partículas lejanas por su masa total y centro de masa.
Si una celda de tamaño característico \(s\) está a distancia \(d\) del cuerpo considerado, el criterio de apertura \(\theta\) de Barnes--Hut~\cite{barnes1986} decide si la celda puede tratarse como un único cuerpo:
\[
