...
El desarrollo de este TFG ha permitido abordar con éxito el desafío de simular la dinámica de una galaxia completa, combinando conocimientos de astrofísica, ingeniería del software y HPC.
Tras finalizar las fases de implementación y experimentación en el supercomputador Finisterrae III, se exponen a continuación las conclusiones principales, la vinculación con la titulación y las posibles líneas de trabajo futuro.

\section{Conclusiones}\label{sec:conclusiones}

El objetivo principal planteado al inicio de este proyecto consistía en diseñar y desarrollar un sistema de simulación de dinámica estelar capaz de superar la barrera de los mil millones de cuerpos del catálogo Gaia DR3, manteniendo un equilibrio riguroso entre la precisión física y la eficiencia computacional.
Tras el análisis de los resultados obtenidos, puedo afirmar que este propósito se ha alcanzado plenamente, logrando procesar la totalidad de la muestra filtrada de la secuencia principal, compuesta por más de 1.061 millones de estrellas.

La implementación del algoritmo jerárquico de Barnes-Hut y su paralelización masiva han sido determinantes para transformar una simulación que teóricamente requeriría siglos mediante fuerza bruta en una tarea ejecutable en tan solo 28 minutos.

Este hito de rendimiento ha sido posible gracias a la convergencia de tres estrategias de optimización crítica.
En primer lugar, la \textbf{reducción algorítmica} de la complejidad cuadrática a logarítmica ($O(N \log N)$) mediante Barnes-Hut, que eliminó la redundancia en el cálculo de fuerzas lejanas.
En segundo lugar, el rediseño de las estructuras de datos para garantizar \textbf{accesos coalescentes a memoria} y evitar la divergencia de hilos (warp divergence) mediante un recorrido de árbol iterativo (\textit{stackless}).
Y finalmente, la \textbf{escalabilidad horizontal} lograda al distribuir la carga de trabajo en 8 GPUs NVIDIA A100, lo que permitió explotar el paralelismo masivo de datos minimizando las transferencias entre host y dispositivo.

Más allá de la consecución de las metas numéricas, el desarrollo de este trabajo ha permitido cumplir con el objetivo fundamental de adquirir un dominio profundo sobre el uso y explotación de infraestructuras HPC.
Este proceso de aprendizaje ha trascendido la programación convencional, exigiendo una inmersión completa en el desarrollo con CUDA para lograr la aceleración masiva en las GPUs NVIDIA A100.
El dominio de esta tecnología ha sido clave para poder llevar el hardware al límite, alcanzando un \textit{speedup} de 126.7x respecto a la ejecución en CPU y permitiendo constatar de primera mano que la ingeniería avanzada es indispensable para abordar retos científicos de escala exa-escalar.

De igual forma, el proyecto ha requerido una capacitación intensiva en la gestión de entornos de supercomputación complejos.
Aprender a utilizar el gestor de colas Slurm en el Finisterrae III ha sido una pieza central, permitiendo orquestar cientos de trabajos, gestionar colas de prioridad y optimizar la asignación de recursos en un entorno de supercomputación real compartido con otros usuarios.
Esta experiencia práctica ha consolidado mis competencias esenciales para operar en centros de datos de alto rendimiento, cerrando la brecha entre la teoría algorítmica y su despliegue en grandes infraestructuras.

Finalmente, se ha garantizado el equilibrio entre la optimización extrema y el rigor científico.
Las pruebas de validación realizadas confirman que las optimizaciones implementadas no han comprometido la fidelidad física, manteniendo el error máximo acotado y verificando la estabilidad orbital del sistema.

En definitiva, el simulador desarrollado constituye una herramienta robusta, eficiente y validada científicamente para el estudio de la dispersión estelar.

\section{Relación con los estudios}\label{sec:relacion-con-los-estudios}

La realización de este TFG ha supuesto la culminación práctica de la formación adquirida durante la titulación, permitiendo integrar y aplicar de forma transversal conocimientos de múltiples disciplinas.
...
Aunque el sistema actual aprovecha al máximo un nodo de computación denso (como el a100-m8 con ocho GPUs), la escala del universo observable invita a expandir el horizonte de simulación más allá de un único nodo.
...
\begin{itemize}
    ...
    Una línea de mejora identificada es la linealización del árbol utilizando \textbf{Morton Codes} o curvas de orden Z~\cite{morton1966}.
    ...
    Para simular sistemas aún mayores o incrementar la resolución temporal, el siguiente paso lógico es la utilización del estándar \textbf{MPI (Message Passing Interface)}.
    ...
    \item \textbf{Tecnologías GPUDirect:}
...
Estas líneas de trabajo futuro permitirían abordar simulaciones cosmológicas completas con una eficiencia aún más alta.
